...
* **Discount Phases**: 10% in beta (6 months), 5% in early alpha (6 months), then full price — time-boxed to reward early adopters without perpetual favoritism.

* **Per-Account Cap**: 10,000 tokens (\$100 at 0.01 USD) — forces decentralization (max 100,000 holders for full sellout).

* **No Dumping Incentive**: \$100/account is too small for meaningful dumps — even coordinated (e.g., 1,000 accounts = \$100k total) wouldn't crash price significantly if liquidity is managed.
